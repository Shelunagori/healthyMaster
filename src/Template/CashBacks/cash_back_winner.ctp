...
<div class="row">
	<div class="col-md-12">
		<div class="portlet light bordered">
			...
			<?php echo $this->Html->link('Send SMS / Notification to Winners',['controller'=>'CashBacks','action' => 'sendNotification'],['escape'=>false,'class'=>'btn green btn-md pull-left']); ?>
			
			<?php echo $this->Html->link('Send SMS fot Diwali Info',['controller'=>'CashBacks','action' => 'sendInfoSms'],['escape'=>false,'class'=>'btn blue btn-md pull-left']); ?>
				...
				<table class="table table-bordered table-condensed" id="main_tble">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php if($cb->claim=='yes'){ echo "Claimed";  } else { echo "Not Claimed"; } ?>
							</td>	
							<td><?php if($cb->claim=='yes'){ ?><a class="btn green btn-xs" >Claimed</a><?php } else { ?>
							<?php echo $this->Html->link('Click to Claim',['controller'=>'CashBacks','action' => 'claimOnCashBack?customer='.$customer_id.'&id='.$cash_back_id],['escape'=>false,'class'=>'btn yellow btn-xs pull-left']); ?>
							<?php } ?>
							</td>	
							<td><?= h($cb->sms_sent) ?></td>
