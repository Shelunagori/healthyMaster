...
class CashBacksController extends AppController
{
	...
	public function sendInfoSms()
    {
		
		$fetch_customers = $this->CashBacks->Customers->find();
		/* 
		foreach($fetch_customers as $customer_details)
		{
			$mobile=$customer_details->mobile;
			
			$sms=str_replace(' ', '+', 'On the auspicious occasion of diwali, our services will remain closed on 19th and 20th October. All order which will be placed during this interval will be delivered on 21st Oct 2017. Team Jain Thela');
			$sms_sender='JAINTE';
			file_get_contents('http://103.39.134.40/api/mt/SendSMS?user=phppoetsit&password=9829041695&senderid='.$sms_sender.'&channel=Trans&DCS=0&flashsms=0&number='.$mobile.'&text='.$sms.'&route=7');
		} 
	...
	public function claimOnCashBack()
    {
		...
		$cash_back_id=$this->request->query('id');
		...
		$this->Flash->success(__('Claimed Successfully.'));
		return $this->redirect(['action' => 'cashBackWinner']);
		exit;	
	...
	public function updateDataStop($id = null)
    {
		$this->viewBuilder()->layout('index_layout'); 
 ...
 exit;
	...
	public function updateWinnersStop($id = null)
    {
		$this->viewBuilder()->layout('index_layout'); 
		$jain_thela_admin_id=$this->Auth->User('jain_thela_admin_id');
